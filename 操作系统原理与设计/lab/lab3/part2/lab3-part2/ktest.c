...
// 定义要输出的文件
#define OUTPUT_FILE "/home/qyniu/code/lab3/lab3.2/expr_result.txt"
...
static void print_mm_active_info(void)
{
    ...
    if (mm)
    {
        ...
        while(vm_tmp)   // 遍历vma
        {
            ...
            for ( ; virt_addr <= vm_tmp->vm_end ; virt_addr += PAGE_SIZE)    // 遍历虚拟地址
            {
                ...
                if (!IS_ERR_OR_NULL(page)) // 检查page是否非空且有效
                {
                    ...
                    if(freq > 1)   // 若被访问到
                    {   // 地址写入
                        record_one_data(((page->index)*PAGE_SIZE)); // page->index是区域内的页索引或是页的线性地址除以PAGE_SIZE
                    }
                }
            }
            vm_tmp = vm_tmp->vm_next;
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    if (pgd_none(*pgd_tmp) || pgd_bad(*pgd_tmp)) 
        return NULL;
    pud_t *pud_tmp = pud_offset((p4d_t *)pgd_tmp,virt);
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        while (vm_tmp) // 遍历vma
        {
			...
			for (virt_addr = vm_tmp->vm_start; virt_addr <= vm_tmp->vm_end ; virt_addr += PAGE_SIZE)
			{   // 页表遍历
				record_two_data(virt_addr, virt2phys(task->mm, virt_addr));
			}
			vm_tmp = vm_tmp->vm_next;
...
static void print_seg_info(void)
{
    ...
    for (addr = mm->start_data; addr < mm->end_data; addr += PAGE_SIZE) // 遍历所有地址
    {
    	...
    	for (virt_addr = vm_tmp->vm_start; virt_addr <= vm_tmp->vm_end ; virt_addr += PAGE_SIZE) // 遍历所有虚拟地址
		{
			struct page *page = mfollow_page(vm_tmp, virt_addr, FOLL_GET); // 得到虚拟地址对应的page
